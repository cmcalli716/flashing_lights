<<<<<<< HEAD
This will be where to put test functions for ROILocator
=======
import os
import unittest
...
class test_ROILocator(unittest.TestCase):
    ...
    def test_diff_of_gauss(self):
        test_img, test_mean, test_std = RL.img_normalizer(sio.imread
                                                          (gray_test_vid))
        test_a = 1
        test_b = 3
        test_c = 0.1
        test_roi = RL.diff_of_gauss(test_img, test_mean, test_std, test_a,
                                    test_b, test_c)
    ...
    def test_ROI_counting(self):
        ...
        test_b = 3
        test_c = 0.1
        test_roi = RL.diff_of_gauss(test_img, test_mean, test_std,
    ...
    def test_ROI_locator(self):
        ...
        test_std = 3
        test_overlap = 0.1
        video_path = RL.ROI_locator(test_vid, data_path, test_mean, test_std,
                                    test_overlap)
        # Test output path existence.
        assert os.path.isfile(video_path),\
            "Video not existing due to output error."
